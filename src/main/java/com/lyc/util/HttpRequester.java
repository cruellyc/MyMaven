...
//		if (this.sessionId == null) {
			...
			if (StringUtils.isNotEmpty(session_value)) {
				this.sessionId = session_value.split(";")[0];
...
//		}
	...
	private HttpRespons makeContent(String urlString,
		...
		try {
			...
			httpResponser.content = setContent(in,
					httpResponser.getContentType());
	 ...
	 * @Description HTTP请求器
	 * @Author JamyLin
	 * @CreateDate 2012-7-24 上午10:44:39
	 */
	public static String request(String urlStr, Charset cs) {
		logger.debug(urlStr);
		URL url;
		String result = "";
		String sCurrentLine = "";
		...
		try {
			...
			if (responseCode == HttpURLConnection.HTTP_OK) {
				...
				BufferedReader bufferedReader = new BufferedReader(
						new InputStreamReader(urlStream, cs));
	...
	public Object setContent(InputStream ins, String contentType) {
		Object result = null;
		HttpRespons httpResponser = new HttpRespons();
		if (contentType.indexOf("image") != -1) {
			try {
				result = FileUtil.inStream2Byte(ins);
			} catch (IOException e) {
				e.printStackTrace();
			}
		} else {
